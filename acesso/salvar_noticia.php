        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="header-section">
                    <h1>Cadastro de Notícias <?php
                        if($_SERVER['REQUEST_METHOD'] == 'POST'){
                            $uploaddir = '../website/noticias/img/';
                            $uploadfile = $uploaddir . ($_FILES['userfile']['name']);

                            if (move_uploaded_file($_FILES["userfile"]["tmp_name"], $uploadfile)) {
                                $array_cad_noticia = array();
                                $array_cad_noticia["img_principal"] = basename($_FILES["userfile"]["name"]);
                                $array_cad_noticia["titulo"] = $_POST["titulo"];
                                $array_cad_noticia["data"] = format_data_us($_POST["data"]);
                                $array_cad_noticia["conteudo"] = $_POST["conteudo"];
                                $cad_noticia = func_cad_noticia($array_cad_noticia);
                                echo $cad_noticia;
                            } else {
